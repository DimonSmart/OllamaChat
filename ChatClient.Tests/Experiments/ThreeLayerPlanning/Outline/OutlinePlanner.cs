...
public sealed class OutlinePlanner(IExperimentLlmClient llmClient)
{
    public async Task<PlannerStageResult<OutlinePlan>> CreatePlanAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return new PlannerStageResult<OutlinePlan>
        {
    ...
    private static void ValidateContract(ExperimentJsonGenerationResult<OutlinePlan> generation)
    {
    ...
    private static void ThrowIfContractFailed(ExperimentJsonGenerationResult<OutlinePlan> generation, List<string> issues)
    {
        if (issues.Count == 0)
            return;
