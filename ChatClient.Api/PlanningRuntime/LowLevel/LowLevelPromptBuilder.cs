...
public static class LowLevelPromptBuilder
{
    public static string BuildSystemPrompt() =>
        ...
        - kind must be exactly one of tool or llm;
        - if kind is tool, capabilityId must be a non-empty string;
        - if kind is llm, out.format must be a non-empty string;
        ...
        IReadOnlyCollection<CompactCapabilitySummary> capabilities)
    {
        ...
        sb.AppendLine(PlanningNodeJson.SerializeIndented(outlinePlan));
        sb.AppendLine();
        sb.AppendLine("Available capabilities:");
        sb.AppendLine(PlanningNodeJson.SerializeIndented(capabilities));
        sb.AppendLine();
