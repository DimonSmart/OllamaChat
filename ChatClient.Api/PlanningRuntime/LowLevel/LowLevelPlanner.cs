...
public sealed class LowLevelPlanner(IPlanningLlmClient llmClient) : ILowLevelPlanner
{
    ...
    private static void ValidateContract(PlanningJsonGenerationResult<LowLevelPlan> generation)
    {
    ...
    private static void ThrowIfContractFailed(PlanningJsonGenerationResult<LowLevelPlan> generation, List<string> issues)
    {
        if (issues.Count == 0)
            return;
