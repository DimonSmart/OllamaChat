    ...
    PlanningCallableAgentCatalog? agentCatalog = null) : IReplanner
{
    ...
    private string BuildSystemPrompt(IReadOnlyCollection<AppToolDescriptor> workflowTools)
    {
        ...
        sb.AppendLine("- The listed capabilities may support any domain or action. Repair from their actual descriptions, schemas, and compatibility hints instead of assuming a fixed workflow.");
        sb.AppendLine("- Do not treat titles, snippets, rankings, ids, shortlist rationales, or other lightweight records as if they were full source content when the user needs precise facts.");
        sb.AppendLine("- If the current plan only has lightweight references or metadata but an available capability can fetch richer source content from them, that is a missing-evidence plan shape problem, not an insufficient-capability conclusion.");
        ...
        sb.AppendLine("- If the failure says there are too few confirmed items or missing source evidence, repair upstream discovery/acquisition first. Add more relevant acquisition calls, refine targeting, or increase a tool limit when metadata says one call may be incomplete; do not try to solve that gap only by loosening the final writer.");
        sb.AppendLine("- If a per-item mapped acquisition step partially fails and the user still needs a fixed number of items, preserve the successful items and use the remaining upstream candidates or broader acquisition to replace the failed ones instead of just rerunning the same failing subset or blocking immediately.");
