    ...
    PlanningCallableAgentCatalog? agentCatalog = null) : IInitialDraftRepairer
{
    ...
    private string BuildSystemPrompt(IReadOnlyCollection<AppToolDescriptor> workflowTools)
    {
        ...
        sb.AppendLine("- Do not treat titles, snippets, rankings, ids, or shortlist rationales as if they were full source content when the user needs precise facts.");
        sb.AppendLine("- If a draft tries to extract or compare precise facts from lightweight records and a listed capability can fetch richer source content from those records, repair upstream by adding that acquisition step instead of only editing prompts or blocking.");
