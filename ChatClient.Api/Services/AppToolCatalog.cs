...
public sealed class AppToolCatalog(IMcpClientService mcpClientService) : IAppToolCatalog
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var clientHandle in clients)
        {
            ...
            if (string.IsNullOrWhiteSpace(serverName) || string.IsNullOrWhiteSpace(baseServerName))
                continue;
            ...
            foreach (var tool in tools)
            {
                ...
                result.Add(new AppToolDescriptor(
                    QualifiedName: bindingId is Guid value && value != Guid.Empty ? $"binding:{value:N}:{toolName}" : $"{baseServerName}:{toolName}",
                    ServerName: serverName,
                    ToolName: toolName,
                    DisplayName: string.IsNullOrWhiteSpace(tool.Title) ? toolName : tool.Title,
                    Description: McpBindingPresentation.BuildToolDescription(tool.Description, clientHandle.Binding),
                    InputSchema: NormalizeInputSchema(tool.JsonSchema),
                    OutputSchema: NormalizeOutputSchema(tool.ReturnJsonSchema),
        ...
        return result.OrderBy(static tool => tool.ServerName, StringComparer.OrdinalIgnoreCase)
            .ThenBy(static tool => tool.ToolName, StringComparer.OrdinalIgnoreCase).ToList();
    }

    private static bool MayRequireUserInput(string? description) =>
        !string.IsNullOrWhiteSpace(description) &&
        (description.Contains("elicitation", StringComparison.OrdinalIgnoreCase) ||
    ...
    private static JsonElement NormalizeInputSchema(JsonElement schema) =>
        schema.ValueKind == JsonValueKind.Object ? schema.Clone() : EmptyObjectSchema.Clone();

    private static JsonElement? NormalizeOutputSchema(JsonElement? schema) =>
        schema is { ValueKind: JsonValueKind.Object } value ? value.Clone() : null;
